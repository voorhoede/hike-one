...
const reload = (model) => {
  ...
  dump
    ...
    .then(() => {
		return confirmReload({ success: true })
			.then(() => console.log(`confirm successful dump to ${DATO_URL}`))
			.catch(err => console.log(`request to dato dump went bad: ${err.message}`))
	})
    .catch(() => {
		return confirmReload({ success: false })
			.then(() => console.warn(`confirm failed dump to ${DATO_URL}`))
	})
