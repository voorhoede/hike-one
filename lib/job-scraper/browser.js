...
module.exports = htmlString => {
	...
	const list = Array.from(jobElements).reduce((list, element) => {
		...
		try {
			title = element.querySelector('.homerun-widget__vacancy__title')
				.textContent;
			duration = element.querySelector('.homerun-widget__vacancy__type')
				.textContent;
			location = element.querySelector('.homerun-widget__vacancy__department')
				.textContent;
		} catch (e) {
			return [...list];
		}
